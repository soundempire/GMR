...
namespace GMR.DAL.Context
{
    internal class DBInitializer : CreateDatabaseIfNotExists<GMRContext>
    {
        protected override void Seed(GMRContext context)
        {
            ...
            var adminPassword = new Password() { ID = 1, Person = admin, Login = "admin", Value = "pIbvhgmpVHahDBTYUgQvew==", LastUpdated = DateTime.Now };
            var abrazumovaPassword = new Password() { ID = 2, Person = abrazumovaPerson, Login = "p1", Value = "LXWtRje9AMTjVuOEllULSG+xJFCUBetWCHhcQlhw/f4=", LastUpdated = DateTime.Now };
            var avdejPassword = new Password() { ID = 3, Person = avdejPerson, Login = "p2", Value = "vDCuPP9IMye6c/gbhVi8zQ==", LastUpdated = DateTime.Now };
